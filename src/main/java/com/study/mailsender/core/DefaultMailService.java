...
/**
 * @description 当邮件发送迅速的时候》》》 会导致邮件服务不可用，为此，故要实现一个限流保证邮件服务能正常发送邮件，将邮件先保存，待服务正常后再发送。
 * 利用生产者消费者模式来重发重要的邮件
 * @Author cxk
...
public class DefaultMailService extends BaseMailService {
            ...
            if (rateLimit.tryAccess()){
                threadPoolExecutor.execute(() -> {
                    try {
                        ...
                        adjustmentMailSender(mailSender);
                        retrySendEmail(mail);
                        Logger.error("=============发送邮件账号挂掉了=============={}",mailSender);
                    }
                });
                break;
            }
    ...
    private void tryRemoveLimit(MailSender mailSender){
        ...
        Logger.error("邮箱账号解封失败！>      {}",mailSender);
        Logger.error("当前账号的等级为 >        {}",mailSender.getSenderRank());
    }
    ...
    private void retrySendEmail(Mail mail){
        // 如果邮件必须到达则加入待发送邮件缓冲池等待消费。
        if (mail.getStrictArrive()){
            Logger.info("邮件重发            {}",mail);
            producer(mail);
        }
    }
    ...
    private void checkParameter(Mail mail){
        ...
        // TODO 目的邮件判空不在检查范围内，调用方自行判定
    }

    /**
     * 生产者，向队列投递消息 TODO 直接线程池的处理不是很好,可能会阻塞发送邮件的线程，全部拿来处理入队操作
     */
        ...
        new Thread(() -> {
            while (true) {
                ...
                try {
                    ...
                    Mail finalMail = mail;
                    threadPoolExecutor.execute(() -> sendMail(finalMail));
                } catch (InterruptedException e) {
                    throw new RuntimeException(e);
                }
                try {
